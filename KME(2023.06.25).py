...
if user_mac_address == default_mac_address:
  ...
  if current_date <= expiration_date: # <<<============================================================
    ...
    import os # 운영 체제와 상호 작용하기 위한 기능을 제공합니다.
    ...
    def open_toplevel(): # <--- 창 종료를 눌렀을 때, 실행되는 함수
    ...
    def images_and_ingredients():
      ...
      if amazon_iherb_value == "amazon": # <------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for url in amazon_urls: # <--- 'check_duplicates_and_add_URL()' 함수에서 반환된 배열입니다.
          ...
          num_a = 0 # <--- 반복문(while)을 위한 'num_a' 초기화
          while True: # <--- 조건이 항상 참(True)이기 때문에 반복문 내부의 구문은 계속해서 반복됩니다.
            css_selector = f"#ivImage_{num_a} > div" # 해당 제품의 모든 사진(동영상 제외)을 수집합니다.
            try: # <--- try 내부의 구문을 실행하다가 예외가 발생하면, except 내부의 구문이 실행됩니다.
              # 수집할 사진을 선택합니다.
              ...
              if naver_checkbox.get(): # <--- 네이버가 선택되었을 경우입니다!
                folder_path = os.path.join(f"../amazon/{asin_code}", "naver(1000, 860)")
                os.makedirs(folder_path, exist_ok=True)
              ...
              if coupang_checkbox.get(): # <--- 쿠팡이 선택되었을 경우입니다!
                folder_path = os.path.join(f"../amazon/{asin_code}", "coupang(500, 780)")
                os.makedirs(folder_path, exist_ok=True)
          ...
          # while
            # try
            except NoSuchElementException: # <--- 원하는 요소를 찾지 못하는 경우, 비정상적으로 종료되는 오류를 방지합니다.
              print("<----- no more image ----->")
              break # <--- 현재 실행되고 있는 반복문(while)을 종료하고 벗어납니다.
        ...
        for url in iherb_urls: # <--- 'check_duplicates_and_add_URL()' 함수에서 반환된 배열입니다.
          ...
          for num_i, img_tag in enumerate(img_tags):
            ...
            if naver_checkbox.get():
              folder_path = os.path.join(f"../iherb/{product_id}", "naver(1000, 860)")
              os.makedirs(folder_path, exist_ok=True)
            ...
            if coupang_checkbox.get():
              folder_path = os.path.join(f"../iherb/{product_id}", "coupang(500, 780)")
              os.makedirs(folder_path, exist_ok=True)
          ...
          translated_text = translated_text.replace("*", "").replace("The standard value per day is not set.", ", ").replace("Nothing.", "")
    ...
    amazon_radiobutton.pack(side="top", fill="x", expand=True, padx=10, pady=(10,2.5))
    ...
    iherb_radiobutton.pack(side="bottom", fill="x", expand=True, padx=10, pady=(2.5,10))
    ...
    password_entry.pack(side="bottom", fill="x", expand=True, padx=20, pady=(2.5,10))
    ...
    frame_03.grid(row=0, rowspan=4, column=1, padx=(5,10), pady=(10,5), sticky="news")
    # ================================================================================
    # 제품 주소 = entry
    url_entry = ctk.CTkEntry(frame_03, placeholder_text=" 제품 주소", font=font_style) # <--- 입력창
    url_entry.pack(fill="x", padx=10, pady=(10,2.5))

    url_scrollable_frame = ctk.CTkScrollableFrame(frame_03, width=500)
    url_scrollable_frame.pack(fill="both", expand=True, padx=10, pady=2.5)
    ...
    product_image_canvas.grid(row=0, column=0, padx=(10,5), pady=(10,5))

    ''''''

    ingredients_textbox_frame = ctk.CTkFrame(frame_04)
    ingredients_textbox_frame.grid(row=0, column=1, padx=(5,10), pady=(10,5), sticky="news")
    ...
    warning_value_lable.pack(padx=5)
    ...
    scrollable_frame.pack(fill="both", expand=True, padx=10, pady=(10,5))
    ...
    frame_07.grid(row=5, column=0, columnspan=2, padx=10, pady=(5,10), sticky="news")
    # ================================================================================
